          ...
          <div className="mt-4 space-y-3">
            {openIssues.length === 0 && warnings.length === 0 ? (
              ...
              [...openIssues, ...warnings].map((check) => (
                <div key={`${check.name}-${check.status}`} className={`rounded-md border p-3 ${statusClass(check.status)}`}>
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <strong className="text-sm">{check.name}</strong>
                    <span className="rounded bg-slate-950/40 px-2 py-1 text-xs">{statusLabel(check.status)}</span>
                  </div>
            ...
            <table className="w-full min-w-[680px] text-left text-sm">
              ...
              <tbody>
                {(latest?.checks ?? []).map((check) => (
                  <tr key={check.name} className="border-b border-slate-800/70">
                    <td className="px-3 py-2 align-top">
                      <span className={`rounded border px-2 py-1 text-xs ${statusClass(check.status)}`}>{statusLabel(check.status)}</span>
                    </td>
                    <td className="px-3 py-2 align-top font-medium text-slate-200">{check.name}</td>
                    <td className="px-3 py-2 align-top text-slate-400">{check.message}</td>
                  </tr>
                ))}
                {!latest && (
                  <tr>
      ...
      <div className="mt-6 rounded-md border border-slate-800 bg-slate-900 p-4">
        ...
        <div className="mt-4 grid gap-2">
          {runs.slice(0, 10).map((run) => (
            <div key={run.id} className="flex flex-wrap items-center justify-between gap-3 rounded border border-slate-800 px-3 py-2 text-sm">
              <div>
                <div className="text-slate-200">{new Date(run.createdAt).toLocaleString()}</div>
                <div className="text-xs text-slate-500">Automated functionality check</div>
              </div>
